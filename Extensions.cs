...
namespace uSource
{
    public static class PathExtension
    {
        ...
        public static string NormalizePath(this string filePath, string ext, string subFolder, bool replaceExtIfNotEqual = true, bool outputWithExt = true)
        {
            ...
            if (filePath != null && filePath.Length > 0)
            {
                ...
                {
                    if (string.IsNullOrEmpty(splits[i]) || ((i == 0 || i == 1) && splits[i] == subFolder)) continue;

                    filePath += i == splitLast
                        ? (splits[i].GetFileExtension(out tempExt))
                        : string.Format("{0}{1}", splits[i], SeparatorChar);
                }
    ...
    public static class OtherExtension
    {
        ...
        public static List<short> ReadAnimationFrameValues(this System.IO.BinaryReader br, int count)
        {
            // Wynikowa lista do przechowywania wartości
            List<short> values = new List<short>();

            // Iteruj przez oczekiwaną liczbę wartości
            for (int i = 0; i < count; /* i jest aktualizowane wewnątrz pętli */)
            {
                // Odczytaj długości skompresowanych i nieskompresowanych danych
                byte[] run = br.ReadBytes(2);
                int compressedLength = run[0];
                int uncompressedLength = run[1];

                // Odczytaj skompresowane wartości
                short[] vals = br.ReadShortArray(compressedLength);

                // Sprawdź poprawność danych
                if (vals.Length != compressedLength)
                {
                    Debug.LogError("Unexpected compressed data length.");
                    throw new InvalidOperationException("Unexpected compressed data length.");
                }

                // Rozpakuj wartości do listy
                for (int j = 0; j < uncompressedLength && i < count; i++, j++)
                {
                    int idx = Math.Min(compressedLength - 1, j); // Upewnij się, że idx nie przekracza długości vals
                    values.Add(vals[idx]);
                }
            }
